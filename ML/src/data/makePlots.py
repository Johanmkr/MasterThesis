...
if __name__ == '__main__':
    ...
    if input("Enter cube manually?") in ["y", "yes", "Y"]:
        ...
        try:
            pk_type = input("Enter power spectrum type (deltacdm(gr only), deltaclass, delta, phi): ") 
        except ValueError: 
    ...
    if plot_avg_ps:
        obj.plot_average_ps(pk_type=pk_type, redshift=redshift, seed_range=np.arange(2000, dtype=int))
    if plot_pot_ps:
