...
def get_cube_path(seed:int, gravity:str, redshift:int) -> pl.Path:
    return simulation_path / f"seed{seed:04d}/{gravity}/{gravity}_{cube.redshift_to_snap[int(redshift)]}_phi.h5"

def get_pickle_path(seed:int, gravity:str, redshift:int, k_res:int, theta_res:int, tag:str) -> pl.PosixPath:
    return analysis_path / "bispectra_dataframes" / f"bispectrum_{gravity}_s{seed:04d}_z{redshift}_k{k_res}_t{theta_res}_tag_{tag}.pkl"
