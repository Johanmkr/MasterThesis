...
class ClassSpectra:
    ...
    def __call__(self, redshift:float=0.0, z2:bool=False) -> np.ndarray:
        z1path, z2path = self._gen_path(redshift)
        if z2:
    ...
    def _gen_path(self, redshift:float) -> tuple:
        redshift_w_comma_separation = f"{redshift:.1f}".replace(".", ",")
        redshift_val = f"comparison_z_{redshift_w_comma_separation}"
