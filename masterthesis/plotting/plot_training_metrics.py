...
metric_path = lambda run, tag, train=True: train_metric_path + f"run-T{run}-tag-{tag}_train.csv" if train else train_metric_path + f"run-T{run}-tag-{tag}_test.csv"
...
def plot_loss():
    ...
    fig, (ax1, ax2) = plt.subplots(2,1, figsize=(15,12), sharex=True, gridspec_kw={'hspace': 0.0})
    
    
    # Plot T1 data
    ax1.set(**plot_settings)
    ax1.plot(T1_train_data["step"], T1_train_data["loss"], color=train_color, lw=2, ls="solid", label="Training")
    ax1.plot(T1_test_data["step"], T1_test_data["loss"], color=test_color, lw=2, ls="solid", label="Testing")
    
    
    
    # plot T2 data
    ax2.set(**plot_settings)
    ax2.plot(T2_train_data["step"], T2_train_data["loss"], color=train_color, lw=2, ls="solid", label="Training")
    ax2.plot(T2_test_data["step"], T2_test_data["loss"], color=test_color, lw=2, ls="solid", label="Testing")
    
    # Add figure legend
    figleg = ax2.legend(loc="upper right")
    
    # Add T1 and T2 text right of y axis
    ax1.text(1.01, 0.5, "T1", fontsize=23, transform=ax1.transAxes, ha="left", va="center")
    ax2.text(1.01, 0.5, "T2", fontsize=23, transform=ax2.transAxes, ha="left", va="center")
...
def plot_rest_of_metrics():
    ...
    fig, axes = plt.subplots(2,2, figsize=(15,12), sharex=True, sharey=True, gridspec_kw={'hspace': 0.0, 'wspace': 0.0})
    
    # Plot T1 data
    for i, tag in enumerate(["F1_score", "accuracy", "precision", "recall"]):
        ax = axes.flatten()[i]
        ax.set(**plot_settings)
        ax.plot(T2_train_data["step"][1:], T2_train_data[tag][1:], color=train_color, lw=2, ls="solid", label="Training")
        ax.plot(T2_test_data["step"], T2_test_data[tag], color=test_color, lw=2, ls="solid", label="Testing")

        # Text with frame
        ax.text(0.85, 0.10, tag.capitalize().replace("_", " "), fontsize=23, transform=ax.transAxes, ha="center", va="top", bbox=dict(facecolor="white", alpha=0.5))
    ...
    figleg = fig.legend(handles=ax.get_lines(), labels=["Training", "Testing"], loc="upper right", ncol=2)
