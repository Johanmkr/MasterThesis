...
class Cube:
    def __init__(self, cube_path: pl.Path, normalise: bool = False) -> None:
        ...
        self.seed = int(
            str(self.cubePath)[-25:-21] if self.gr else str(self.cubePath)[-33:-29]
        )  # Will be 0000, 0001, etc.
    ...
    ###TODO Fix the below function to take the dimension into account.
    ...
    def __getitem__(self, idx: int | tuple | slice | list) -> np.ndarray:
        """
    ...
    def __str__(self) -> str:
        ...
        """
        RT = "\nCube Information\n----------------------------\n"
        RT += f"Path: {'/'.join(str(self.cubePath).split('/')[-3:])}\n"
...
if __name__ == "__main__":
    ...
    path = (
        ...
        + f"seed{seed_nr}/"
        + gravity
        + f"/{gravity}_{redshift_to_snap[redshift]}_phi.h5"
    )
