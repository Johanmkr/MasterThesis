...
class CubeBispectrum(cube.Cube):
    ...
    def squeezed_bispectrum(self, k_range:np.array, kwargs:dict={"threads": 10}) -> pd.DataFrame:
        """
        ...
        for i, k in enumerate(k_range):
            ...
            theta = 19/20 * np.pi
            BBk = PKL.Bk(self.data, self.boxsize, k, k, np.array([theta]), "CIC", **kwargs)
            #TODO:  the dimensionality below
            B[i] = BBk.B #* k**3 / (2*np.pi**2)
            Q[i] = BBk.Q #* k**3 / (2*np.pi**2)
        dBk = pd.DataFrame({"k": k_range, "B": B, "Q": Q})
        return dBk
    
    def whole_cube_spectrum(self, k_res:int=25, theta_res:int=25, kwargs:dict={"threads": 24}) -> None:
        ...
        for i, k1_val in enumerate(k1_range):
            for j, k2_val in enumerate(k2_range):
                ...
                actual_iterations += 1
                try:
                    BBk = PKL.Bk(self.data, self.boxsize, k1_val, k2_val, theta_range, "CIC", **kwargs)
                    B[i,j,1:-1] = BBk.B
                    Q[i,j,1:-1] = BBk.Q
        ...
        dBk = pd.DataFrame({"k1": k1.flatten(),
                            "k2": k2.flatten(),
                            "k3": k3.flatten(),
                            "theta": theta.flatten(),
                            "B": B.flatten(),
