...
datapath = "/mn/stornext/d10/data/johanmkr/simulations/prepared_data/" + f"scaled_data_A_s{2.215e-9:.3e}.h5"

total_seeds = np.arange(0,300)

output_path = "/mn/stornext/d10/data/johanmkr/simulations/prepared_data/pk_from_scaled_data.h5"
...
def calculate_Pks():
    with h5py.File(output_path, "w") as f:
        for seed in tqdm(total_seeds, desc="Calculating Pks"):
            ...
            f.create_dataset(f"{seed}/gr", data=np.array(grPKs), dtype=np.float32, compression="gzip")
            f.create_dataset(f"{seed}/newton", data=np.array(newtonPKs), dtype=np.float32, compression="gzip")
